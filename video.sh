...
# ffmpeg -vaapi_device /dev/dri/renderD128 -hwaccel_output_format vaapi -i "$1" -filter:v format='nv12,hwupload',scale_vaapi="-8:'min(720,ih)'" -c:v hevc_vaapi -qp 30 -c:a aac -b:a 96k "$2"
